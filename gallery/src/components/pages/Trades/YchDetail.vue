<template>
    <div class="adopt-page" v-if="this.ychData">
        <div class="hero-bg" :style="heroStyle">
            <div class="hero-inner">
                <img :src="this.ychData.previewImage" alt="preview" class="hero-art" />
                <div class="meta">
                    <h1 class="name">{{ this.ychData.title }}</h1>
                    <p class="by">Автор: <strong>{{ this.ychData.userName }}</strong></p>
                    <p class="short">{{ this.ychData.short }}</p>
                    <div class="actions">
        ...
        <main class="content">
            <section class="adopt-panel preview">
                ...
                <div v-html="this.ychData.description"></div>
            </section>
...
export default {
    ...
    methods: {
        ...
        async getYchData() {
            try {
                const response = await axios.get('https://furry-world.ru/console/ych/get_ych_detail.php?id=' + this.$route.params.id + '&code=' + this.$route.params.ychcode, {
                    headers: { "Authorization": "Bearer " + localStorage.getItem("token") }
                });
                this.ychData = response.data;
                this.gallery = this.ychData.gallery || [];
                this.prices = this.ychData.prices || { fix: 0, min: 0, ab: 0 }
            } catch (error) {
                console.log('Error fetching YCH data:', error);
            }
        },
        thumbStyle(i) {
            const tilt = (i - 1) * 1
